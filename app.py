...
class Config:
    ...
    SUPPORT_BOT_USERNAME = "MyTrustYieldSupport_bot"
    SECRET_KEY = "mytrustyield_secret_key_2024"
...
# ========= قاعدة البيانات المحسنة مع الإصلاحات =========
def get_conn():
...
def initialize_existing_users():
    ...
    try:
        ...
        if users_without_membership:
            ...
            c.execute("SELECT value FROM meta WHERE key='last_membership_id'")
            result = c.fetchone()
            current_id = int(result[0]) if result else 862
            
            for user_row in users_without_membership:
                user_id = user_row[0]
                c.execute("UPDATE users SET membership_id = ? WHERE user_id = ?", (current_id, user_id))
                current_id += 1
            
            c.execute("UPDATE meta SET value = ? WHERE key = 'last_membership_id'", (str(current_id),))
            conn.commit()
...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            last_deposit TEXT,
            last_task TEXT,
            withdraw_request INTEGER DEFAULT 0,
            transactions TEXT,
            referrer_id TEXT,
            created_date TEXT,
            last_login TEXT,
            ...
            last_task_status TEXT DEFAULT 'completed'
        )
    """)
    ...
    try:
        c.execute("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_users_membership_id'")
        if not c.fetchone():
            c.execute("CREATE UNIQUE INDEX idx_users_membership_id ON users(membership_id)")
            logger.info("✅ Created unique index for membership_id")
    except Exception as e:
    ...
    c.execute("INSERT OR IGNORE INTO meta (key, value) VALUES ('user_count', '0')")
    c.execute("INSERT OR IGNORE INTO meta (key, value) VALUES ('last_membership_id', '861')")
    
    indexes = [
        ("idx_users_username", "users(username)"),
        ("idx_deposits_status", "deposit_requests(status)"),
        ("idx_users_balance", "users(balance)"),
        ("idx_users_referrer", "users(referrer_id)"),
        ("idx_deposits_txid", "deposit_requests(txid)"),
        ("idx_users_status", "users(status)"),
        ("idx_referral_batch_referrer", "referral_batch_bonus(referrer_id)"),
        ("idx_user_bans_user_id", "user_bans(user_id)"),
        ("idx_user_bans_status", "user_bans(status)"),
        ("idx_user_bans_end_time", "user_bans(ban_end_time)")
    ]
    ...
    optimize_database()
    logger.info("✅ Database initialized successfully with enhanced features")
...
def get_next_membership_id():
    ...
    try:
        ...
        c.execute("REPLACE INTO meta (key, value) VALUES ('last_membership_id', ?)", (str(next_id),))
        conn.commit()
        return next_id
...
def load_user(user_id):
    """تحميل بيانات المستخدم مع التخزين المؤقت - نسخة معدلة"""
    ...
    keys = [col[1] for col in columns_info]
    ...
    for i, key in enumerate(keys):
        if i < len(row):
            ...
            if key == "membership_id":
                ...
                user[key] = None
            elif key == "login_count":
                user[key] = 0
            elif key == "status":
                user[key] = "active"
            elif key == "first_deposit_time":
                user[key] = None
            elif key == "last_withdrawal_time":
                user[key] = None
            elif key == "last_task_status":
                user[key] = "completed"
            else:
                user[key] = None
    ...
    user["registered"] = bool(user.get("registered", 0))
    user["deposited"] = bool(user.get("deposited", 0))
    user["withdraw_request"] = bool(user.get("withdraw_request", 0))
    
    transactions_data = user.get("transactions")
    if transactions_data and isinstance(transactions_data, str):
        try:
            user["transactions"] = json.loads(transactions_data)
        except:
            user["transactions"] = []
    else:
        user["transactions"] = []
...
def save_user(user):
    ...
    c.execute("""
        REPLACE INTO users (user_id, membership_id, username, password, email, phone, wallet, balance,
                          registered, deposited, last_deposit, last_task, withdraw_request, 
                          transactions, referrer_id, created_date, last_login, login_count, status, first_deposit_time, last_withdrawal_time, last_task_status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        ...
        int(user.get("registered", False)),
        int(user.get("deposited", False)), 
        user.get("last_deposit"), 
        user.get("last_task"),
        int(user.get("withdraw_request", False)), 
...
def validate_phone_number(phone):
    ...
    # نمط لرقم الهاتف مع رمز الدولة (مثال: +201234567890 أو 00201234567890)
...
def is_txid_unique(txid):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT 1 FROM deposit_requests WHERE txid = ?", (txid,))
...
def user_exists(username, exclude_id=None):
    ...
    c = conn.cursor()
    if exclude_id:
        c.execute("SELECT 1 FROM users WHERE username=? AND user_id!=?", (username, str(exclude_id)))
    else:
...
def wallet_exists(wallet, exclude_id=None):
    ...
    c = conn.cursor()
    if exclude_id:
        c.execute("SELECT 1 FROM users WHERE wallet=? AND user_id!=?", (wallet, str(exclude_id)))
    else:
        c.execute("SELECT 1 FROM users WHERE wallet=?", (wallet,))
    exists = c.fetchone() is not None
    conn.close()
    return exists
...
def is_user_banned(user_id):
    ...
    c.execute("""
        ...
        WHERE user_id = ? AND status = 'active' AND ban_end_time > ?
    """, (str(user_id), now))
...
def ban_user(user_id, admin_id, ban_duration, ban_reason="تم الحظر من قبل الإدارة"):
    ...
    try:
        ...
        try:
            ban_message = f"""
🚫 **تم حظر حسابك من قبل إدارة MyTrustYield**
...
نأسف للإزعاج ونأمل أن تكون فترة الحظر فرصة للالتزام بشروط الاستخدام.
            """
            bot.send_message(user_id, ban_message, parse_mode="Markdown")
        except Exception as e:
...
def unban_user(ban_id, admin_id):
    ...
    try:
        ...
        c.execute("SELECT * FROM user_bans WHERE id = ?", (ban_id,))
        ...
        # تحديث حالة الحظر
        c.execute("UPDATE user_bans SET status = 'inactive' WHERE id = ?", (ban_id,))
        conn.commit()
        conn.close()
...
نرحب بعودتك إلى MyTrustYield ونتطلع إلى توفير أفضل تجربة استثمارية لك.

شكراً لتفهمك والتزامك بشروط الاستخدام.
            """
            bot.send_message(ban_record[1], unban_message, parse_mode="Markdown")
        except Exception as e:
            logger.error(f"Error sending unban message to user {ban_record[1]}: {e}")
...
def get_active_bans():
    """الحصول على جميع الحظور النشطة - نسخة مصححة"""
    try:
        conn = get_conn()
        c = conn.cursor()
        ...
        c.execute("""
            SELECT * FROM user_bans 
            WHERE status = 'active' AND ban_end_time > ?
...
def check_ban_expiry():
    ...
    try:
        ...
        for ban in expired_bans:
            ...
            c.execute("UPDATE user_bans SET status = 'expired' WHERE id = ?", (ban[0],))
...
شكراً لتفهمك والتزامك بشروط الاستخدام.
                """
                bot.send_message(ban[1], expiry_message, parse_mode="Markdown")
            except Exception as e:
                logger.error(f"Error sending ban expiry message to user {ban[1]}: {e}")
...
def has_pending_deposit(user_id):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT 1 FROM deposit_requests WHERE user_id=? AND status='pending'", (str(user_id),))
    exists = c.fetchone() is not None
    conn.close()
    return exists
...
def add_withdrawal(user_id, amount):
    conn = get_conn()
    c = conn.cursor()
    c.execute("INSERT INTO withdrawals (user_id, amount, status, date) VALUES (?, ?, ?, ?)",
...
def get_referral_batch_bonus(referrer_id):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT * FROM referral_batch_bonus WHERE referrer_id=?", (str(referrer_id),))
    row = c.fetchone()
    conn.close()
    ...
    return {
        ...
        'pending_users': json.loads(row[3]) if row[3] else [],
        'total_bonus_earned': row[4],
...
def update_referral_batch_bonus(referrer_id, completed_batches=None, pending_users=None, total_bonus_earned=None):
    ...
    c.execute("""
        ...
        SET completed_batches=?, pending_users=?, total_bonus_earned=?, last_bonus_date=?
        WHERE referrer_id=?
    """, (
...
"""
    ...
    bot.send_message(referrer_id, notification_text, parse_mode="Markdown")
...
class MessageTemplates:
    ...
    def welcome_message():
        return """
🌟 **مرحباً بك في MyTrustYield - منصة الاستثمار الذكية** 🌟
...
def require_login(func):
    def wrapper(message, *args, **kwargs):
        ...
        if not security.check_rate_limit(user_id, "login_check", 10, 60):
            bot.send_message(message.chat.id, "⏳ تم تجاوز الحد المسموح للمحاولات. يرجى الانتظار قليلاً.")
            return
        
        if user_id not in logged_in_users:
            bot.send_message(message.chat.id, "🔐 **يجب تسجيل الدخول أولاً**\n\nالرجاء تسجيل الدخول للوصقل هذه الميزة.")
            return
...
def has_pending_withdrawal(user_id):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT 1 FROM withdrawals WHERE user_id=? AND status='pending'", (str(user_id),))
    exists = c.fetchone() is not None
    conn.close()
    return exists
...
def daily_task(message):
    ...
    if ban_check['banned']:
        bot.send_message(message.chat.id, "🚫 حسابك محظور ولا يمكنك استخدام هذه الميزة.")
        return
...
@{Config.SUPPORT_BOT_USERNAME}
        """
        bot.send_message(message.chat.id, ban_message, parse_mode="Markdown")
        return
...
💰 **اختر من القائمة أدناه لمواصلة رحلتك الاستثمارية:**
    ...
    if success:
        bot.send_message(message.chat.id, MessageTemplates.welcome_message(), 
                        parse_mode="Markdown",
                        reply_markup=main_menu(message.from_user.id))
...
def register_start(message):
    ...
    if not security.check_rate_limit(message.from_user.id, "registration", 3, 300):
        bot.send_message(message.chat.id, "⏳ يمكنك محاولة التسجيل مرة أخرى بعد 5 دقائق.")
        return
...
def login_start(message):
    ...
    if not user:
        bot.send_message(message.chat.id, "❌ الحساب غير موجود. الرجاء التسجيل أولاً.")
        return
    
    if not user.get("registered"):
        bot.send_message(message.chat.id, "❌ الحساب غير مكتمل. الرجاء إكمال التسجيل.")
        return
...
def forgot_credentials(message):
    ...
    if not user or not user.get("registered"):
        bot.send_message(message.chat.id, "⚠️ لا يوجد حساب مرتبط بهذا المستخدم.")
        return
...
def logout(message):
    ...
    bot.send_message(message.chat.id, 
                    ...
                    "شكراً لاستخدامك MyTrustYield! نراك قريباً 🚀",
                    parse_mode="Markdown",
...
def handle_support_category(message):
    ...
    if message.text == "🏠 العودة للقائمة الرئيسية":
        ...
        bot.send_message(user_id, "✅ العودة للقائمة الرئيسية", reply_markup=main_menu(user_id))
        return
...
def handle_support_message(message):
    ...
    if not message.text or len(message.text.strip()) < 5:
        bot.send_message(user_id, "❌ الرسالة قصيرة جداً، يرجاء كتابة تفاصيل أكثر.")
        return
...
def handle_cancel_and_home_buttons(message):
    ...
    if message.text == "❌ إلغاء العملية":
        ...
        bot.send_message(message.chat.id, "✅ تم إلغاء العملية.", reply_markup=main_menu(user_id))
...
def handle_state(message):
    ...
    if state == "await_login_username":
        ...
        conn = get_conn()
        c = conn.cursor()
        c.execute("SELECT * FROM users WHERE user_id=? AND username=?", 
        ...
        if not user:
            bot.send_message(message.chat.id, "⚠️ حدث خطأ، حاول مرة أخرى.", reply_markup=main_menu())
            user_states.pop(message.from_user.id, None)
            return
        ...
        if not user:
            bot.send_message(message.chat.id, "⚠️ حدث خطأ، حاول مرة أخرى.")
            return
...
def deposit_start(message):
    ...
    if not security.check_rate_limit(message.from_user.id, "deposit", 5, 60):
        bot.send_message(message.chat.id, "⏳ تم تجاوز الحد المسموح. يرجى الانتظار قليلاً.")
        return
...
def handle_deposit_txid(message):
    ...
    if not user:
        bot.send_message(message.chat.id, "⚠️ حدث خطأ، أعد العملية.", reply_markup=main_menu(user_id))
        user_states.pop(user_id, None)
        return
...
def handle_deposit_sender_wallet(message):
    ...
    try:
        c.execute(
            "INSERT INTO deposit_requests (user_id, username, amount, network, txid, status, date, sender_wallet) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=processing_msg.message_id,
            text="❌ **خطأ في النظام: رقم العملية مكرر**\n\n"
                 "لقد تم استخدام هذا الرقم مسبقاً في النظام.\n"
                 "يرجى التواصل مع الدعم الفني.",
            parse_mode="Markdown"
        )
        logger.error(f"Duplicate TXID despite pre-check: {txid}")
    except Exception as e:
        bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=processing_msg.message_id,
...
def dashboard_info(message):
    ...
    if not user:
        bot.send_message(message.chat.id, "❌ لم يتم العثور على بيانات المستخدم.")
        return
    
    conn = get_conn()
    c = conn.cursor()
    
    c.execute("SELECT COUNT(*) FROM users WHERE referrer_id=?", (str(user_id),))
    referred_count = c.fetchone()[0]
    
    c.execute("""
        SELECT COUNT(*) FROM users u 
        JOIN deposit_requests dr ON u.user_id = dr.user_id 
        WHERE u.referrer_id=? AND dr.status='approved'
    """, (str(user_id),))
    ...
    c.execute("SELECT SUM(amount) FROM deposit_requests WHERE user_id=? AND status='approved'", (str(user_id),))
    total_deposits = c.fetchone()[0] or 0
    
    c.execute("SELECT SUM(amount) FROM withdrawals WHERE user_id=? AND status='approved'", (str(user_id),))
...
def withdraw_request(message):
    ...
    if not withdrawal_check['eligible']:
        bot.send_message(message.chat.id, withdrawal_check['message'], parse_mode="Markdown")
        return
...
def handle_wallet_confirmation(message):
    ...
    try:
        c.execute(
            "INSERT INTO withdrawals (user_id, amount, status, date) VALUES (?, ?, ?, ?)",
...
def referral_link(message):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM users WHERE referrer_id=?", (str(message.from_user.id),))
    referred_count = c.fetchone()[0]
    ...
    c.execute("""
        ...
        WHERE u.referrer_id=? AND dr.status='approved' AND u.deposited=1
    """, (str(message.from_user.id),))
    total_deposits = c.fetchone()[0] or 0
    ...
    markup = telebot.types.InlineKeyboardMarkup()
    share_button = telebot.types.InlineKeyboardButton("📤 مشاركة الرابط", url=f"https://t.me/share/url?url={link}&text=انضم%20إلى%20MyTrustYield%20للحصول%20على%20أفضل%20الاستثمارات!")
    markup.add(share_button)
    
    bot.send_message(
        message.chat.id,
        f"👥 **نظام الإحالة - MyTrustYield**\n\n"
        f"🔗 **رابط الإحالة الخاص بك:**\n"
        f"`{link}`\n\n"
        f"📊 **إحصائيات أدائك:**\n"
        f"• 👥 **عدد المُحالين:** {referred_count}\n"
        f"• ✅ **المحالين النشطين:** {active_referrals}\n"
        f"• 💰 **إجمالي أرباحك:** {referral_earnings:.2f}$\n\n"
        f"🏆 **جوائز الإحالة الجماعية:**\n"
        f"• 📦 **المجموعات المكتملة:** {batch_bonus_data['completed_batches']}\n"
        f"• 👥 **المحالين في الانتظار:** {batch_bonus_data['pending_users_count']}/3\n"
        f"• 💵 **إجمالي الجوائز:** {batch_bonus_data['total_bonus_earned']:.2f}$\n"
        f"• 🎯 **المتبقي للجائزة القادمة:** {batch_bonus_data['users_until_next_bonus']} مستخدم\n\n"
        f"💰 **مكافآت الإحالة:**\n"
        f"• 🎯 5% من أول إيداع لكل مستخدم جديد\n"
        f"• 🏆 100$ لكل 3 مستخدمين جدد يقومون بالإيداع\n"
        f"• 💵 المكافآت تضاف تلقائياً لرصيدك\n"
        f"• 📈 كلما زاد عدد المحالين زادت أرباحك\n\n"
        f"📣 **كيفية الاستفادة:**\n"
        f"1. شارك الرابط مع أصدقائك\n"
        f"2. عند تسجيلهم عبر رابطك\n"
        f"3. وعند أول إيداع لهم\n"
        f"4. تحصل على 5% من إيداعهم + 100$ لكل 3 مستخدمين\n\n"
        f"🚀 **ابدأ بجني الأرباح الآن!**",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
def explain(message):
    terms_text = """
📜 **الشروط والأحكام - MyTrustYield** 📜

🏛️ **مقدمة عامة:**
MyTrustYield هي منصة استثمارية رقمية مرخصة وخاضعة للرقابة، تقدم خدمات استثمارية آمنة وموثوقة عالمياً. نحن ملتزمون بتوفير بيئة استثمارية شفافة وآمنة لجميع عملائنا.
...
📞 **خاتمة:**
نشكرك على اختيار MyTrustYield كشريكك الاستثماري. نحن ملتزمون بتوفير أفضل الخدمات وضمان تجربة استثمارية آمنة ومربحة لجميع عملائنا.

**📅 تاريخ آخر تحديث: 1 يناير 2024**
**🏢 MyTrustYield - منصة الاستثمار الذكي الموثوقة عالمياً**
    """
...
def admin_panel(message):
    if message.from_user.id != Config.ADMIN_ID:
        bot.send_message(message.chat.id, "❌ لا يوجد صلاحية للوصول لهذه اللوحة.")
        return
    ...
    bot.send_message(message.chat.id, 
                    "👨‍💼 **لوحة تحكم الإدارة - MyTrustYield**\n\n"
...
def comprehensive_stats(message):
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM users WHERE deposited=1")
        active_users = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM users WHERE date(created_date)=date('now')")
        ...
        c.execute("SELECT COUNT(*) FROM support_messages WHERE date(created_at)=date('now')")
        ...
        c.execute("SELECT COUNT(*) FROM user_bans WHERE date(created_at)=date('now')")
        new_bans_today = c.fetchone()[0]
        
        conn.close()
        
        stats_text = f"""
📊 **الإحصائيات الشاملة - MyTrustYield**
...
• 💰 إجمالي التداول: {total_deposit_amount + total_withdrawal_amount:.2f}$
        ...
        bot.send_message(message.chat.id, "❌ حدث خطأ في توليد الإحصائيات.")
...
def handle_broadcast_message(message):
    if message.text == "❌ إلغاء العملية":
        ...
        bot.send_message(message.chat.id, "✅ تم إلغاء الإرسال.")
        return
...
def show_pending_deposits(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات إيداع معلقة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"📋 **الطلبات المعلقة:** {len(requests)} طلب")
...
def show_approved_deposits(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات إيداع مقبولة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"✅ **الطلبات المقبولة:** {len(requests)} طلب")
...
def show_rejected_deposits(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات إيداع مرفوضة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"❌ **الطلبات المرفوضة:** {len(requests)} طلب")
...
def show_all_deposits(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات إيداع حالياً.")
        return
    ...
    bot.send_message(message.chat.id, f"📊 **جميع طلبات الإيداع:** {len(requests)} طلب")
...
def show_pending_withdrawals(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات سحب معلقة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"💸 **طلبات السحب المعلقة:** {len(requests)} طلب")
...
def show_approved_withdrawals(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات سحب مقبولة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"✅ **طلبات السحب المقبولة:** {len(requests)} طلب")
...
def show_rejected_withdrawals(message):
    ...
    if not requests:
        bot.send_message(message.chat.id, "✅ لا توجد طلبات سحب مرفوضة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"❌ **طلبات السحب المرفوضة:** {len(requests)} طلب")
...
def show_all_withdrawals(message):
    ...
    bot.send_message(message.chat.id, f"📊 **جميع طلبات السحب:** {len(requests)} طلب")
...
def handle_member_search(message):
    if message.text == "❌ إلغاء العملية":
        user_states.pop(f"admin_{message.from_user.id}", None)
        bot.send_message(message.chat.id, "✅ تم إلغاء البحث.")
        return
    ...
    # البحث باسم المستخدم
    c.execute("SELECT * FROM users WHERE username LIKE ? OR user_id=? OR membership_id=?", 
             (f"%{search_term}%", search_term, search_term))
    users = c.fetchall()
    
    if not users:
        bot.send_message(message.chat.id, "❌ لم يتم العثور على أي عضو بهذا المعيار.")
        conn.close()
        return
    
    bot.send_message(message.chat.id, f"🔍 **نتائج البحث:** {len(users)} عضو")
...
def handle_ban_search(message):
    ...
    if len(users) > 1:
        bot.send_message(message.chat.id, f"🔍 **وجدت {len(users)} عضو، الرجاء تحديد المستخدم بدقة أكثر.**")
        conn.close()
        return
...
def handle_ban_duration(message):
    if message.text in ["❌ إلغاء العملية", "🏠 العودة للقائمة الرئيسية"]:
        ...
        bot.send_message(message.chat.id, "✅ تم إلغاء عملية الحظر.", reply_markup=main_menu(message.from_user.id))
        return
    ...
    if not ban_duration:
        bot.send_message(message.chat.id, "❌ الرجاء اختيار مدة حظر صحيحة من القائمة.")
        return
...
def unban_member_start(message):
    ...
    try:
        ...
        if not active_bans:
            bot.send_message(message.chat.id, "✅ لا توجد حسابات محظورة حالياً.")
            return
...
📝 **سبب الحظر:** {ban['ban_reason']}
        ...
        bot.send_message(message.chat.id, f"❌ حدث خطأ في عرض الحسابات المحظورة: {e}")
...
def show_open_support_messages(message):
    ...
    if not messages:
        bot.send_message(message.chat.id, "✅ لا توجد رسائل دعم مفتوحة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"📨 **الرسائل المفتوحة:** {len(messages)} رسالة")
...
def show_answered_support_messages(message):
    ...
    if not messages:
        bot.send_message(message.chat.id, "✅ لا توجد رسائل دعم مجابة حالياً.")
        return
    
    bot.send_message(message.chat.id, f"✅ **الرسائل المجابة:** {len(messages)} رسالة")
...
def show_all_support_messages(message):
    ...
    if not messages:
        bot.send_message(message.chat.id, "✅ لا توجد رسائل دعم حالياً.")
        return
    ...
    bot.send_message(message.chat.id, f"📊 **جميع رسائل الدعم:** {len(messages)} رسالة")
...
✅ **النظام يعمل بشكل طبيعي ولا يحتاج لصيانة طارئة**
    """.format(
        len(logged_in_users),
        task_queue.get_active_count(),
        len(notification_manager.pending_notifications),
        len(get_active_bans())
    )
    
    bot.send_message(message.chat.id, maintenance_text, parse_mode="Markdown")
...
def approve_deposit_request(call, req_id):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT * FROM deposit_requests WHERE id=?", (req_id,))
    req = c.fetchone()
    ...
    if user:
        ...
        try:
            bot.send_message(
                ...
                f"شكراً لاستخدامك MyTrustYield! 📈",
                parse_mode="Markdown"
            )
        except Exception as e:
...
def handle_deposit_rejection_reason(message):
    ...
    if req:
        ...
        c.execute("UPDATE deposit_requests SET status='rejected', reject_reason=? WHERE id=?", (reject_reason, req_id))
        conn.commit()
...
نأسف للإزعاج ونتطلع لخدمتك في طلبات مستقبلية.
                ...
                logger.info(f"✅ Deposit rejection notification sent to user {user_id}, message ID: {sent_msg.message_id}")
                rejection_sent = True
                break
...
للإستفسار: @{Config.SUPPORT_BOT_USERNAME}
                """
                bot.send_message(int(user_id), simple_rejection)
                logger.info(f"✅ Alternative rejection message sent to user {user_id}")
                rejection_sent = True  # 🔥 أضف هذا السطر الجديد هنا
            except Exception as e2:
...
def approve_withdrawal_request(call, withdraw_id):
    ...
    if user and user.get("balance", 0) >= amount:
        ...
        c.execute("UPDATE withdrawals SET status='approved', admin_id=?, processed_date=? WHERE id=?", 
...
def request_withdrawal_rejection_reason(call, withdraw_id):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT w.*, u.username FROM withdrawals w JOIN users u ON w.user_id = u.user_id WHERE w.id=?", (withdraw_id,))
    withdrawal = c.fetchone()
    conn.close()
...
def handle_withdrawal_rejection_reason(message):
    ...
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT w.*, u.username, u.user_id FROM withdrawals w JOIN users u ON w.user_id = u.user_id WHERE w.id=?", (withdraw_id,))
    withdrawal = c.fetchone()
    
    if withdrawal:
        w_id, user_id, amount, status, admin_id, processed_date, date, tx_hash, old_reject_reason, username, user_id = withdrawal
        
        c.execute("UPDATE withdrawals SET status='rejected', reject_reason=? WHERE id=?", (reject_reason, withdraw_id))
        conn.commit()
...
نأسف للإزعاج ونتطلع لخدمتك في طلبات مستقبلية.
                ...
                logger.info(f"✅ Withdrawal rejection notification sent to user {user_id}, message ID: {sent_msg.message_id}")
                rejection_sent = True
                break
...
def close_support_message(call, msg_id):
    ...
    c.execute("UPDATE support_messages SET status='closed' WHERE id=?", (msg_id,))
    conn.commit()
    conn.close()
...
def handle_support_reply(message):
    ...
    c.execute("SELECT * FROM support_messages WHERE id=?", (msg_id,))
    ...
    if support_msg:
        ...
        c.execute("UPDATE support_messages SET status='closed', admin_response=?, responded_at=?, admin_id=? WHERE id=?", 
...
def generate_daily_report():
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM users WHERE date(created_date)=?", (today,))
        new_users = c.fetchone()[0]
        
        c.execute("SELECT COUNT(*) FROM deposit_requests WHERE date(date)=? AND status='approved'", (today,))
        deposits_today = c.fetchone()[0]
        
        c.execute("SELECT SUM(amount) FROM deposit_requests WHERE date(date)=? AND status='approved'", (today,))
        deposits_amount = c.fetchone()[0] or 0
        
        c.execute("SELECT COUNT(*) FROM withdrawals WHERE date(date)=?", (today,))
        withdrawals_today = c.fetchone()[0]
        
        total_members = get_total_membership_count()
        
        c.execute("SELECT COUNT(*) FROM referral_batch_bonus WHERE date(last_bonus_date)=?", (today,))
        batch_bonus_today = c.fetchone()[0]
        
        c.execute("SELECT SUM(total_bonus_earned) FROM referral_batch_bonus WHERE date(last_bonus_date)=?", (today,))
        batch_bonus_amount = c.fetchone()[0] or 0
        
        # إحصائيات الحظر
        c.execute("SELECT COUNT(*) FROM user_bans WHERE date(created_at)=?", (today,))
        new_bans_today = c.fetchone()[0]
...
• حسابات محظورة جديدة: {new_bans_today}
        ...
        try:
            bot.send_message(Config.ADMIN_ID, report, parse_mode="Markdown")
        except:
            pass
...
if __name__ == "__main__":
    try:
        if not os.path.exists(Config.DB_FILE):
            logger.info("🗄️ إنشاء قاعدة بيانات جديدة")
            init_db()
        else:
            logger.info("✅ قاعدة البيانات موجودة بالفعل - متابعة التشغيل")
            init_db()
        
        print("=" * 50)
        print("🤖 **MyTrustYield Bot - Enhanced Version**")
        print("🚀 **منصة الاستثمار الذكية المحسنة**")
        print(f"📅 تاريخ التشغيل: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"👥 إجمالي الأعضاء: {get_total_membership_count()}")
        print(f"🔧 الأنظمة المحملة: Cache, Task Queue, Notifications, Ban System")
        print("=" * 50)
        
        threading.Thread(target=schedule_backups, daemon=True).start()
        threading.Thread(target=schedule_optimization, daemon=True).start()
        threading.Thread(target=schedule_daily_reports, daemon=True).start()
        threading.Thread(target=schedule_ban_check, daemon=True).start()
        ...
        try:
            bot.send_message(Config.ADMIN_ID, f"🚨 **تحطم البوت**\n\nالخطأ: {str(e)}")
        except:
            pass
